...
A standalone GraphQL client library for MyMarketManager, compatible with MAUI mobile apps, Blazor WASM, Blazor Server, and other .NET applications.

## Overview

This library provides a strongly-typed GraphQL client for accessing the MyMarketManager GraphQL API. It uses [StrawberryShake 15](https://chillicream.com/docs/strawberryshake) to generate type-safe client code from the GraphQL schema.
...
MyMarketManager.GraphQL.Client/
├── Generated/
│   └── MyMarketManagerClient.Client.cs   # Generated client code
├── GraphQL/
│   └── products.graphql                   # GraphQL operation definitions
├── .graphqlrc.json                        # StrawberryShake configuration
├── schema.graphql                         # Generated schema (not committed)
└── MyMarketManager.GraphQL.Client.csproj
```
...
**Note**: The WebApp project uses this approach and registers the client even though Razor pages currently use DbContext directly. This is preparation for future migration.
...
**Generated Method:**
```csharp
IOperationResult<ICreateProductResult> result = 
    await client.CreateProduct.ExecuteAsync(input, cancellationToken);
```
...
**Generated Method:**
```csharp
IOperationResult<IUpdateProductResult> result = 
    await client.UpdateProduct.ExecuteAsync(productId, input, cancellationToken);
```
...
**Generated Method:**
```csharp
IOperationResult<IDeleteProductResult> result = 
    await client.DeleteProduct.ExecuteAsync(productId, cancellationToken);
```
...
```json
{
  ...
  "extensions": {
    "strawberryshake": {
      "name": "MyMarketManagerClient",
      "namespace": "MyMarketManager.GraphQL.Client.Generated",
...
- [MyMarketManager GraphQL Server README](../MyMarketManager.WebApp/GraphQL/README.md)
